...
load_agent() {
    if [ -f "$AGENT_FILE" ]; then
        ...
        if ! kill -0 $SSH_AGENT_PID 2>/dev/null; then
            ...
            echo "♠️ Agent alive with PID: $SSH_AGENT_PID"
        fi
    else
...
case "$1" in
    ...
    status)
        if [ -f "$AGENT_FILE" ]; then
            source "$AGENT_FILE"
            if kill -0 $SSH_AGENT_PID 2>/dev/null; then
                echo "♠️ SSH Agent running (PID: $SSH_AGENT_PID)"
            else
...
if [ -f "$HOME/.ssh/pixel-agent/agent-wrapper.sh" ]; then
    source <("$HOME/.ssh/pixel-agent/agent-wrapper.sh" load)
fi
...
alias ssh-agent-reload='$HOME/.ssh/pixel-agent/agent-wrapper.sh load'
EOF
...
add_all_keys() {
    "$HOME/.ssh/pixel-agent/agent-wrapper.sh" load
...
case "$1" in
    ...
    add-all)
        ...
        ;;
    *)
        echo "Usage: $0 {ed25519|rsa|add-all}"
