...
# Check if key and at least one field-value pair is provided
if [ "$#" -lt 3 ]; then
    echo "Usage: $0 <stream-key> <field> <value> [field2 value2 ...]"
    echo "Example: $0 garden.diary event \"Planted new roses\" time \"2:30 PM\""
    exit 1
fi

STREAM_KEY="$1"
shift  # Remove the first argument (stream key)

# Build the field-value pairs string
FIELDS=""
while [ "$#" -gt 1 ]; do
    FIELD="$1"
    VALUE="$2"
...
CMD="redis-cli -h \"$REDIS_HOST\" -p \"$REDIS_PORT\" -a \"$REDIS_PASS\" --no-auth-warning XADD $STREAM_KEY * $FIELDS"
echo "Executing: $CMD"
eval "$CMD"
